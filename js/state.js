...
function defaultState() {
  return {
    ...
    settings: {
      ...
      blueBonnetProxyUrl: "",
      blueBonnetCheckins: false, blueBonnetCheckinHours: 3, lastBlueBonnetCheckinAt: null,
...
function loadState() {
  try {
    ...
    // Real, pre-existing local data saved before cross-device sync existed
    // won't have an updatedAt yet (defaults to 0). Leaving it at 0 would
    // make it look OLDER than literally anything ever pushed to Drive —
    // including an earlier, emptier sync from a different device — and get
    // silently overwritten. Stamp it "now" instead so real local data is
    // never mistaken for the oldest/least-authoritative copy.
