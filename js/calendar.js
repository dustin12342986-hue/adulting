...
const Calendar = (function () {
  ...
  function connect() {
    return new Promise((resolve, reject) => {
      ...
      let settled = false;
      // Google's sign-in popup can silently fail to open (blocked by the
      // browser) or fail to hand a result back — with no error and no
      // callback firing at all, which used to leave connect() hanging
      // forever with no feedback. If nothing happens within 20s, treat it
      // as a failure so the user gets an actual message instead of silence.
      const timeoutId = setTimeout(() => {
        if (settled) return;
        settled = true;
        reject(new Error("Google's sign-in window didn't open or was closed — this is usually your browser blocking pop-ups. Check your browser's address bar for a \"pop-up blocked\" icon (or Site settings), allow pop-ups for this site, and try Connect again."));
      }, 20000);
      tokenClient.callback = (resp) => {
        ...
        // GIS reports exactly which scopes the user actually granted in
        // resp.scope (space-separated) — trust that over just having *a*
        // token, since a token can come back valid but missing the scope
        // we actually needed (this is what silently broke Drive sync
        // before: a real token, just not one with Drive access on it).
        const granted = (resp.scope || "").split(/\s+/);
        if (!granted.includes("https://www.googleapis.com/auth/drive.appdata")) {
          return reject(new Error("Google signed you in but didn't grant Drive access this time. Click Disconnect, then Connect again, and make sure the Drive permission checkbox is checked (not unchecked) on the approval screen before continuing."));
        }
